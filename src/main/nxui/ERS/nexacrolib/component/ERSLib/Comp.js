...
*******************************************************************************
● gfnGetCommonCode         : 공통코드콤보설정(Combo/Grid)	callbackFunc ->afterCommCodeLoad
● gfnGetCommonCodeEx         : 공통코드콤보설정 서버조회방식	callbackFunc -> fnAfterCommCodeLoad
● gfnFindData         : dataSet object에서 키에 해당되는 Row를 찾아서 rowpostion 전달
● gfnGetLookupData         : dataSet object에서 키에 해당되는 Row를 찾아서 칼럼값을 전달
● gfnGetTextSize         : text 넓이 및 높이를 구하는 함수
*/
...
pForm.gfnGetCommonCode = function(param, sLocale)
{
    ...
    for(var idx in param)
	{
        if(!this.gfnIsNull(param[idx].objid))
		{
			...
			if(param[idx].objid instanceof Combo)
			{
				param[idx].objid.innerdataset = dsComboName;
				param[idx].objid.codecolumncodecol;

				if(this.gfnIsNull(param[idx].data))
				{
					param[idx].objid.datacolumn = "SSC_CD_KORN_NM";
				}
				else
				{
					param[idx].objid.datacolumn = param[idx].data;
				}
				//2016.08.04 공통코드 셋팅이전에 값이 없을 경우에 index=0 을 반영한다.
				if(this.gfnIsNull(param[idx].objid.value))
					param[idx].objid.index = 0;

			}
			else if(param[idx].objid instanceof Radio)
			{
				var bEnabe = param[idx].objid.enable;
				param[idx].objid.enable = true;
				param[idx].objid.innerdataset = dsComboName;
				//2016.08.11 KSK 잠금처리되어있던 부분 다시 활성화 처리함
				param[idx].objid.codecolumn = codecol;
//                param[idx].objid.codecolumn = "COMM_CODE";
                if(this.gfnIsNull(param[idx].data))
				{
					param[idx].objid.datacolumn = "SSC_CD_KORN_NM";
				}
				...
				{
					param[idx].objid.columncount = dsComboName.rowcount;
				}
				else if(direction == "vertical")
				{
					param[idx].objid.direction = "vertical";
				}
				if(!bEnabe)
				{
					param[idx].objid.enable = bEnabe;
				}
			}
...
pForm.gfnGetCommonCodeEx = function(param)
{
	...
	for(var idx in param)
	{
		if(!this.gfnIsNull(param[idx].objid))
		{
			...
			{
				case "Combo" :
					param[idx].objid.displayrowcount = 20;
				case "Radio" :
					param[idx].objid.innerdataset = dsComboName;
					param[idx].objid.codecolumn = codecol;
					param[idx].objid.datacolumndata;
