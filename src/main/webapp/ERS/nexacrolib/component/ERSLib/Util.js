...
● gfnLengthByte           : 문자열 byte 길이를 조회
● gfnGetDigit             : 문자열 byte 길이를 조회
● gfnLPad                 : 왼쪽 문자 채우기
● gfnRPad                 : 오른쪽 문자 채우기
● gfnRound                : 소수점 반올림을 한다.
		//-----Date 관련 Util-----//
...
● gfnGetDayKor            : 입력된 날자로부터 요일을 구함
● gfnIsDate               : 날짜 여부를 확인한다.(년월 or 년월일)
● gfnGetLastDate          : 년월을 입력받아 마지막 일를 반환한다(년월)
● gfnGetBirthYear         : 주민번호 뒷 첫번째 자리로 년대를 return 한다.
● gfnDateToWeek           : 입력받은 date로 week(주차)를 리턴한다.
● gfnCalculateWeek        : 입력받은 date로 week(주차)를 리턴한다.
● gfnGetWeekCountOfMonth  : 날짜(Date) 해당 월의 최대 주수(주차) 구하기
● gfnGetDiffDate          : 두 일자간의 차이 일수 계산
		//-----validation check용-----//
● gfnChkByteLen           : 문자열 Byte 길이를 체크한다
● gfnIsString             : 문자형 여부 체크
● gfnIsDigit              : 숫자체크
● gfnIsKoreanChar         : 한글만으로 되어 있는지 Check한다.
● gfnIsSpecialChar        : 특수문자가 있는지 Check한다.
● gfnIsEng                : 문자열이 영문인지 Check한다.
● gfnIsEngNum             : 문자열이 영문+숫자인지 Check한다.
● gfnHasNumber            : 문자열에 숫자가 포함되어 있는지 체크한다.
● gfnHasKor               : 문자열에 한글이 포함되어 있는지 체크한다.
● gfnHasEng               : 문자열에 영문이 포함되어 있는지 체크한다.
...
● gfnIsPhoneNum           : 전화번호or핸드폰 번호를 체크한다.
● gfnIsTelNum             : 전화번호를 체크한다.
● gfnIsMobileNum          : 핸드폰번호를 체크한다.
● gfnChkLenPeriod         : 문자열 최소 및 최대 길이 범위를 체크한다.
● gfnChkByteLenPeriod     : 문자열 Byte 최소 및 최대 길이 범위를 체크한다.
...
● gfnReplaceAll           : 문자열을 치환을 한다.
● gfnRemoveTag            : HTML TAG 제거 함수
● gfnIsArray              : Array 여부 반환
● gfnEequals              : 주어진 두개의 인자에 대하여 동일한 값인지 확인한다.
● gfnIsBoolean            : value의 boolean 여부 반환.
● gfnIsDate2              : value의 Date 여부 반환.
● gfnIsNumber             : value의 number 여부 반환.
● gfnIsObject             : value의 Object 여부 반환.
● gfnIsXComponent         : value의 nexacro component 여부 반환.
● gfnIsFunction           : value의 Function 여부 반환.
● gfnIsEmpty              : value의 빈값 여부 반환.
● gfnIsEmpty2             : 파라미터로 여러개의 Object 를 검사할 때 하나라도 empty 이면 true 리턴한다.
● gfnIsNotEmpty2          : not empty 여부 확인.
● gfnGetObjectType        : 오브젝트 유형을 문자열로 리턴한다.
● gfnTypeOf               : nexacro 객체의 type 반환.
● gfnGetFormFindObj       : form의 object가 있으면 해당 object를 반환하는 함수
● getElementId            : 제네레이트된 넥사크로 컴포넌트 id 반환
● getJQueryId             : JQuery ID  반환
*/
